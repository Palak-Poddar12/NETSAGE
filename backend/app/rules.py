    ...
    for out in outputs:
        for m in re.finditer(r'(\d+\.\d+\.\d+\.\d+)', out):
            try:
                ...
                findings.append({"rule_id": "INVALID_IP", "category": "IP", "severity": "HIGH", "message": f"Invalid IP {m.group(1)}", "evidence": "IP address format is invalid"})
    return findings
...
def check_interface_down(outputs: List[str]) -> List[Dict]:
    ...
    for out in outputs:
        for line in out.splitlines():
            ...
            elif "down" in low and "up" not in low and "line protocol" in low:
                iface = line.split()[0] if line.split() else "unknown"
